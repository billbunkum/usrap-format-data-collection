...
def logs(new_df, vida_keys): ## WORKING HERE
  ...
  ### Give me the KEY for Key in vida_kesy only if Key is in new_df.columns
...
if file_format == 'spatial':
  ...
  new_df[0].to_csv(f'{new_filename}--CODED-FOR-ViDA.csv', index=False) 
  new_df[1].to_csv(f'{new_filename}--MISSING-CELLS-LOG.csv')
#  vida_batch.to_csv(f'{new_filename}--coded-from-spatial.csv', index=False)

# Accepts SR4D but adds 2 extra ArcGIS cols
if file_format == 'sr4d':
  vida_batch.to_csv(f'{new_filename}--coded-from-sr4d-plus-2-arcCols.csv', index=False)
