...
def number_of_lanes(): ## ANCHOR / WORKING HERE
  ...
  if file_format == 'sr4d':
    ...
    # Apply correct fields from 'batch' to 'vida_batch'
      ...
      print("You must add f'{cardinal}' and f'{total_num}' Columns to SR4D formatted file.'\n") 
      print('Exiting program...')
      sys.exit()
...
# Gather needed elements from 'vida_batch' into an SR4D format
...
if file_format == 'spatial':
  new_df = build_sr4d_csv()
  new_df.to_csv(f'coded-sr4d--{new_filename}.csv', index=False) 
  vida_batch.to_csv(f'coded-spatial--{new_filename}.csv', index=False)

# Accepts SR4D with extra ArcGIS cols
if file_format == 'sr4d':
  vida_batch.to_csv(f'coded-sr4d-2-arcCols--{new_filename}.csv', index=False)
